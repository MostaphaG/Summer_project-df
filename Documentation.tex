...
\section{Graphical representations of 2 forms on $\mathbb{R}^{2}$}
\subsection{2 form representations - blocks and stacks}
Two alternative ways of representing 2 forms have been considered. The first, simpler method considers the 2 form magnitude at different points in space and represents them as squares, whose size is determined by the magnitude, and whose colour determines the orientation, obtained by the sign of the 2 form at that point.
...
This issue was found to be fundamental. As clear and intuitive as this method is for graphically representing 2 forms, it does not provide a unique way of doing so. TO be precise, at the very least, no such method has yet been found. In fact, all the representations it does provide are elements of a rotation group ($SO(2)$) of the resulting 2 form. To present this idea more clearly, lets look at the more rigorous proof of the above stated non-uniqueness.
...
There exists yet another way of defining 2 forms through stacks that represent 1 forms. This includes splitting the 2 form into components, not into $\wedge dy$ and $dy\wedge dx$ that are understood to originate from the exterior derivative, but into $f(x, y)dx \wedge g(x, y)dy$, where the 2 form is $f(x,y)g(x,y) dx\wedge dy$. This was not coded, as it yields another issue. Similarly to the last considered method, the splitting of the 2 form into a product of two functions is not unique. Again, as above for the previous method, it is also possible here to let the user input two 1 forms to wedge together into a 2 form, but once again, this may also include cancellations or merges between components, which make the 2 form not unique.

\subsection{2 form representation as stacks in code}
Due to the fact that unique representations could not be deduced. The code we produced allows the user to choose, what representation they would like to see.

\textbf{User-supplied 2 forms} are being plotted as stacks by splitting the expression into two components $dx\wedge dy$ and $dy\wedge dx$. The splitting of the expression is set by the user via the 'set splitting' button (by default splitting occurs into two equal halves). These are then being plotted as previously stated, one as the $x$ component and the other as the $y$ component. This plotting is done via a function ('form\textunderscore 2\textunderscore components\textunderscore plot') which is very similar in operation to stack\textunderscore plot. The new function however, also uses the end 2 form expression, evaluates it numerically, finds its sign and depending on this, colours the stacks. It was chosen to colour via the convention that grey signifies zero magnitude at that position, red represents positive (ccw) 2 form magnitude and blue stands for negative (cw) magnitude.


\textbf{2 forms from the exterior derivative} are plotted by a similar process as above, with the initial addition of finding the mathematical expression of the 2 form and simplifying it (as has been described above). Once the 2 form is found, it is input into the entry box for 2 forms and is highlighted in light green to display that this is currently being displayed (as opposed to other options including the supplied 1 forms).


\textbf{2 forms through the wedge product} is completed by first asking the user to input two 1 forms that are to be wedged together, in terms of their components in $dx$ and $dy$ (other on $\mathbb{R}^{3}$). This is done by displaying a new window for these inputs after the 'wedge' button is pressed. Once these are submitted this extra window is closed, although its entries are saved and display again when it is re-opened. The inputs are simplified by briefly passing them into 'SymPy' expressions. The components of these inputs are then calculated using the previously described 'eq\textunderscore to\textunderscore comps'. The 2 form is then found by concatenating strings of input in such a way as to result in the correct 2 form. This is also simplified in the same way and displays in the 2 form entry box, which, once again, light up to a light green. The plotting of superposing stacks is then completed by using the stack\textunderscore plot function with disabled arrowheads.

Given $w_{1} = f(x, y) dx + g(x, y) dy$  and $w_{2} = h(x, y) dx + m(x, y) dy$. The graphical representation must be made up of:


\begin{equation}
	\label{T2} \begin{split}
		f(x, y) dx\wedge m(x, y) dy + &h(x, y) dy\wedge g(x, y) dx \\
		= \\
...
which is executed when the if condition in the response function is satisfied.
Two rectangular stacks with the scaling factor are accounted to by giving similar colours to the vertical  and horizontal stacks (red and green). After the first rectangular stacks are added the second group will either sit on top of the first (in which case scaling contribution is zero) or sit in some gaps, hence increasing the density as result of its scaling function.
If any of the coefficient functions (f, g, h or m)  is zero, the stacking reduces to one function together with its elemental 2 form $dx\wedge dy$, these are executed in the 'elif' options under the if statement.


\subsection{2 form representation as blocks in code}
The simpler method is also implemented to show the comparison between the  locally detailed, non-unique (stacks( and locally undefined, global, unique (blocks).
This is more simply implemented by defining the above described process of plotting blocks in a single function and calling it with the appropriate 2 form whenever necessary.
The option as to which should be used is given to the user through radio buttons, however, when stacks is selected, a message box displays, warning the user of the non-uniqueness of the result.
...
To accomodate for that and a few other issues mentioned below, the plotting now is done by using a newly defined function 'form\textunderscore 1\textunderscore to\textunderscore 2onR3', which works in a similar manner to the stacks\textunderscore plot. With the mentioned extra operations.
