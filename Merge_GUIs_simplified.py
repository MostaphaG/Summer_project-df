...
def stack_plot(xg, yg, axis, F_x, F_y, s_max, L, pt_den, fract, arrows=False, stacks=True, orientation='mid', scale=1, w_head=1/8, h_head=1/4, axis_check=0, arrowheads=True, colour='green', check_2_frm=0, s_min=2, logartmic_scale_bool=0):
    ...
    if stacks is True:
        ...
        # define the points that set out a line of the stack sheet (middle line)
        A_x = xg + 0.5*s_L*I_sin
        A_y = yg - 0.5*s_L*I_cos
        B_x = xg - 0.5*s_L*I_sin
        B_y = yg + 0.5*s_L*I_cos
        
        # define points of stack arrowheads as arrays for all stacks
        p_sh1x = xg + 0.5*s_L*I_cos + (s_L*w_head)*I_sin
        p_sh1y = yg + 0.5*s_L*I_sin - (s_L*w_head)*I_cos
        p_sh2x = xg + 0.5*s_L*I_cos - (s_L*w_head)*I_sin
        p_sh2y = yg + 0.5*s_L*I_sin + (s_L*w_head)*I_cos
        ...
        for n in range(1, s_max+1):
            ...
            # List of the indexes of elements with n sheets
            n_list = []
            for i in range(x_len):
                for j in range(y_len):
                    if R_int[i,j] == n:
            ...
            if n == 1:
                ...
                for (i,j) in n_list:
                    ...
                    if arrowheads == True:
                        ...
                        axis.add_line(Line2D((P_sh1x[i,j], P_sh3x[i,j]), (P_sh1y[i,j], P_sh3y[i,j]), linewidth=1, color = colour))
                        axis.add_line(Line2D((P_sh2x[i,j], P_sh3x[i,j]), ((P_sh2y[i,j], P_sh3y[i,j])), linewidth=1, color = colour))
            
            # If more than one sheet,
            elif n > 1:
                
                # If odd, add middle line
                if J == 1:
                    for (i,j) in n_list:
                        axis.add_line(Line2D((A_x[i,j], B_x[i,j]), (A_y[i,j], B_y[i,j]), linewidth=1, color=colour))
